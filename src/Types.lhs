...
> data UnactivatedLayer = UL {ulSize :: Int, ulWeights :: [NrnWeight], ulActivation :: Activation} 
> data ActivatedLayer   = AL {alSize :: Int, alWeights :: [NrnWeight], alActivation :: Activation, inputs :: [NrnSignal], outputs :: [NrnSignal], derivs :: [Derivative]} 
> data BackpropLayer    = BL {blSize :: Int, blWeights :: [NrnWeight], blActivation :: Activation, errors :: [NrnSignal]}
...
> data Activation = AD ([NrnSignal] -> [(NrnSignal, Derivative)]) | ND ([NrnSignal] -> [NrnSignal])
...
To help with using and debugging the code, layers should also be printable. In the future, activation functions will be more strongly typed and made instances of the Show class so that they can be printed along with the layers.

> instance Show UnactivatedLayer where
>     show (UL sz ws _) = "UL {ulSize = " ++ show sz ++ ", ulWeights = " ++ show ws ++ ", ulActivation = ..." ++ "}"
> instance Show ActivatedLayer where
>     show (AL sz ws _ is os ds) = "AL {alSize = " ++ show sz ++ ", alWeights = " ++ show ws ++ ", inputs = " ++ show is ++ ", outputs = " ++ show os ++ ", derivs = " ++ show ds ++ "}" 
> instance Show BackpropLayer where
>     show (BL sz ws _ es) = "BL {blSize = " ++ show sz ++ ", blWeights = " ++ show ws ++ ", errors = " ++ show es ++ "}"